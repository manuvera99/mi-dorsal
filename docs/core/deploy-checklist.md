...
- **⚠️ IMPORTANTE — Vercel NO despliega Convex automáticamente**: el `buildCommand` de `vercel.json` es solo `next build`, nunca `convex deploy`. Cada vez que se mergea a `pre` un cambio que toca `convex/` (nueva función, índice, schema), hay que ejecutar MANUALMENTE `npx convex deploy --env-file <archivo-con-CONVEX_DEPLOY_KEY-de-pre>` — si no, el frontend de PRE sigue funcionando pero las queries/mutations que dependen del cambio fallan con `Server Error` genérico (sin detalle) porque el backend de PRE sigue en el código viejo. Confirmado 2026-09-11: `getSeasonRankingPlaceholder` fallaba porque faltaban los índices `by_club`/`by_club_active`/`by_profile` de `clubMemberships` en el deployment aislado tras mergear el fix `821513d`.
